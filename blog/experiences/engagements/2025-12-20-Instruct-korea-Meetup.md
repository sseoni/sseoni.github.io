...
이 프로젝트는 AI 에이전트가 방대한 정보 속에서 학습 목표(Outcome)를 중심으로 지식을 구조화(Ontology)하고, 사용자의 지식 갭(Gap)을 찾아 채워주는 모델을 제안합니다. 궁극적으로는 서로의 지식을 공유할 수 있는 **‘지식의 Github’**을 구축하는 것이 목표입니다.

이번 발표에서는 "에이전트가 다양한 글을 MECE하게 구조화할 수 있는가?", "사용자가 이를 자신의 지식 지도로 관리할 수 있는가?"라는 핵심 가설을 공유했습니다. 현장에서 만난 엔지니어, 리서처분들의 피드백 덕분에 프로젝트의 방향성(Direction)을 더 구체화할 수 있는 뜻깊은 시간이었습니다.
