...
코드는 그나마 주석이라도 있지만, `git log`를 열었을 때 `fix`, `update`, `aaa` 같은 커밋 메시지만 가득하다면? 이는 스파게티 코드와 다를 바가 없다.
...
Tip: 터미널에서 git commit -m "메시지" 대신 git commit만 입력하면 텍스트 에디터가 열려 여러 줄을 편하게 쓸 수 있다.
